...
async def on_member_ban(guild: discord.Guild, user: Union[discord.User, discord.Member]):
    ...
    try:
        ...
        try:
            make_api_request(OPCode.DISABLE_USER, {"user_id": user.id})
...
async def on_member_update(old: discord.Member, new: discord.Member):
    ...
    if VERIFIED_ROLE not in new._roles:
        ...
        try:
            make_api_request(OPCode.DISABLE_USER, {"user_id": new.id})
        except HTTPException as e:
        ...
        try:
            make_api_request(OPCode.ENABLE_USER, {"user_id": new.id})
        except HTTPException as e:
...
async def sync(ctx: commands.Context):
    ...
    try:
        ...
        make_api_request(OPCode.DISABLE_USER_BULK, {"user_ids": [ban.user.id for ban in banned_users]})
...
async def user_count(ctx: commands.Context):
    try:
        await ctx.defer()
        count = make_api_request(OPCode.USER_COUNT)
...
async def key_count(ctx):
    try:
        count = make_api_request(OPCode.KEY_COUNT)
...
async def key_search(ctx: commands.Context, query):
    ...
    try:
        results = make_api_request(OPCode.SEARCH, {"query": query})
        if not results:
...
async def enable_user(ctx: commands.Context, user: discord.User):
    ...
    try:
        log_channel = await bot.fetch_channel(LOG_CHANNEL_ID)
        try:
            make_api_request(OPCode.ENABLE_USER, {"user_id": user.id})
        except HTTPException as e:
